...
Given one structurally valid `ProofCertificateV0` and the currently accepted
parent state, Ledger V0 applies this order:
...
2. mathematically check that normal form exactly once while resolving every
   reachable `ProofReference` exclusively from the unchanged parent state;
...
4. classify the statement as `New` when its `StatementId` is absent from the
   parent state, otherwise as `Existing`;
...
The candidate proof is not visible during step 2. It therefore cannot cite
itself or a future proof. A reference succeeds only after its exact `ProofId`
was accepted by an earlier transition.
...
- the first accepted derivation of a statement is `New`;
- a genuinely different accepted derivation of the same statement is
  `Existing`;
- an existing `ProofId` or `DerivationId` is a registration error and produces
  no novelty result.

This classification does not calculate a reward and does not claim general
mathematical equivalence between structurally different statements. A later
block policy may use `New` for theorem novelty while applying separate fees and
resource limits.

## Atomicity and errors

Normalization and mathematical checking do not mutate the parent state.
...
A future `BlockV0` contains exactly one proof. It must bind that proof to one
parent block and validate it against exactly that parent's ledger state. A
proof accepted on another fork or merely received from the network is not part
of that state.
