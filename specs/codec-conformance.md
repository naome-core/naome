...
| Canonical state request/response envelope | `naome-protocol/src/state_exchange/tests.rs`; `naome-network/src/transport/state_exchange/tests.rs` and `tests/boundary.rs` | Genesis/profile/direction/version checks before allocation; exact EOF and frame/proof limits; two-frame custody charge; cancelled/truncated reads release permits |
