...
- ECON-084 [DECIDED] [NOT_IMPLEMENTED] Each fee-funded signer entitlement equals `floor(validator_fee_pool * signer_weight / total_active_agreement_weight)` at the prior height, and every validator-fee-pool atom not assigned by that formula is burned in the same settlement; AFTER: `PROD-032`, `ECON-083`.
