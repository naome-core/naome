...
use naome_consensus::research::ResearchBranch;
use naome_research::{ResearchState, profile::Genesis};
...
pub async fn run(args: &[String]) -> Result<()> {
    if args[0] == "verify" || args[0] == "inspect" {
        ...
        for height in 1..=manifest.height {
            ...
            {
                let record = naome_research::ResearchRecord::decode(
                    finality.proposal().record_bytes(),
                    &genesis,
                )?;
                for operation in record.operations() {
                    if let naome_research::operations::OperationBody::Reveal { original, .. } =
                        naome_research::operations::OperationBody::decode(
                            operation.payload(),
                            &genesis,
                        )?
                    {
        ...
        if args[0] == "inspect" {
            let id = naome_research::OperationId::from_bytes(files::unhex(&args[3])?);
            ...
            if let Some(naome_research::state::FamilyResult::Completed {
                normalization_receipt,
                ..
            }) = branch.state().families().get(&q.question().resolution_id())
            {
                let receipt = naome_research::receipt::NormalizationReceipt::decode(
                    normalization_receipt,
                    &genesis,
                )?;
