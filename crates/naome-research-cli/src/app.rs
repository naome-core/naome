...
async fn run(args: Vec<String>) -> Result<()> {
    match args.first().map(String::as_str) {
        ...
        Some("compile-question") if args.len() == 3 => {
            let genesis = naome_research::profile::Genesis::decode(&files::read(
            ...
            let question =
                naome_research::question::CompiledQuestion::compile(&source, genesis.profile())?;
        ...
        _ => Err(
            "usage: naome-research setup DIRECTORY lab|research|short-test RUN_RECORDS BASE_PORT"
                .into(),
        ),
    }
}
