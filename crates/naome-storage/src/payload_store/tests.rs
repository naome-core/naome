...
impl PayloadStoreIo for ScriptedIo {
...
fn append_barriers_are_ordered_and_every_ambiguous_failure_reopens_old_or_new() {
    ...
    let mut success = ProofPayloadStoreCore::empty(ScriptedIo::new(None), policy);
    assert_eq!(
    ...
    faults.extend((0..=DIGEST_BYTES as usize).map(|after| Fault::Write {
    ...
    for fault in faults {
        let mut core = ProofPayloadStoreCore::empty(ScriptedIo::new(Some(fault.clone())), policy);
        assert!(
            matches!(
                core.insert(record),
