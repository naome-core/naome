...
fn every_incomplete_second_entry_cut_recovers_exact_first_block() {
    ...
    for cut in first_image.len() + 1..complete.len() {
        ...
        assert!(journal.artifact(first_id).unwrap().is_some(), "cut={cut}");
        assert_eq!(
...
fn complete_payload_and_footer_corruption_fail_without_recovery() {
    ...
    assert!(matches!(
        ...
        Err(ArtifactChainJournalError::Replay { entry: 0, .. })
    ));
...
fn prefix_and_chain_context_are_never_recovered_as_a_tail() {
    ...
    assert!(matches!(
        ArtifactChainJournal::open_recovering_unverified(&directory.path, wrong_definition),
...
fn complete_framing_errors_fail_closed() {
    ...
    assert!(matches!(
        ArtifactChainJournal::open_recovering_unverified(&directory.path, definition),
        Err(ArtifactChainJournalError::InvalidEntryLength { entry: 0, actual, .. })
            if actual == ENTRY_MIN_BODY_BYTES - 1
    ));

    let invalid_block_body = vec![0_u8; ENTRY_MIN_BODY_BYTES as usize];
    let invalid_block_entry =
        raw_entry(&invalid_block_body, ArtifactBlockId::from_bytes([0_u8; 32]));
    let directory = TestDirectory::new();
...
fn block_id_valid_parent_payload_and_entry_order_attacks_fail_replay() {
    ...
    let id = definition.id();
    let (payloads, artifact_ids) = dependency_chain_with_len(1);
    let valid = one_block(definition, artifact_ids[0]);

    let stale = ArtifactBlock::new(
        ArtifactBlockId::from_bytes([0xaa; 32]),
        valid.previous_artifact_set_root(),
        valid.resulting_artifact_set_root(),
        valid.artifact_id(),
    );
    assert_replay_parent_failure(
        definition,
        vec![(stale, payloads[0].clone(), artifact_ids[0])],
    );

    let directory = TestDirectory::new();
    let substituted = axiom_bytes(ZfcAxiom::Union);
    let swapped_image = journal_image(id, &[(valid, substituted, artifact_ids[0])]);
...
fn verified_open_rejects_rollback_and_does_not_truncate_untrusted_tail() {
    ...
    assert!(ArtifactChainJournal::open_recovering_unverified(&directory.path, definition).is_ok());
    assert!(matches!(
        ArtifactChainJournal::open_verified(&directory.path, definition, full_head),
...
fn assert_replay_parent_failure(
    ...
    assert!(matches!(
        ArtifactChainJournal::open_recovering_unverified(&directory.path, definition),
        Err(ArtifactChainJournalError::Replay { source, .. })
            if matches!(
                source.as_ref(),
