...
impl ResearchSigner {
    ...
    pub fn sign_time_report(
        &self,
        utc_seconds: u64,
    ) -> Result<naome_research::time::SignedTimeReport, Error> {
        self.ensure()?;
        Ok(naome_research::time::SignedTimeReport::sign(
    ...
    pub fn retained_quorum(
        &self,
    ) -> Result<Option<&naome_consensus::research::ResearchQuorum>, Error> {
        self.ensure()?;
