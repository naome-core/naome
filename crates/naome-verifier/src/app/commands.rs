...
use naome_network::FinalityProofRequest;
...
pub(super) fn execute(
    ...
    match command {
        ...
        Command::Import {
            ...
            .map_err(Rejected)?;
            let route = route(&envelope, journal, None)?;
            let payload = files::bytes(&base.join(payload_file), ARTIFACT_PAYLOAD_MAX_BYTES)
                .map_err(Rejected)?;
            commit(&envelope, payload, route, journal)
        }
    }
}

// File imports and authenticated network responses meet at the same complete
// verifier and sealed-commit path. A request address is an additional constraint,
// never a substitute for deriving the parent and verifying all evidence.
pub(super) fn import_response(
    envelope: &[u8],
    payload: Vec<u8>,
    expected: FinalityProofRequest,
    journal: &mut Journal,
) -> std::result::Result<(Value, bool), Failure> {
    let route = route(envelope, journal, Some(expected))?;
    if payload.len() > ARTIFACT_PAYLOAD_MAX_BYTES {
        return Err(Failure::Rejected("payload_length"));
    }
    commit(envelope, payload, route, journal)
}
...
fn route(
    envelope: &[u8],
    journal: &Journal,
    expected: Option<FinalityProofRequest>,
) -> std::result::Result<ConsensusValueV0, Failure> {
    use Failure::{Fatal, Rejected};
    ...
    let route = ConsensusValueV0::from_canonical_bytes(&envelope[..ConsensusValueV0::BYTE_LENGTH])
        .map_err(|_| Rejected("envelope_value"))?;
    if route.context() != journal.context() {
        return Err(Rejected("envelope_context"));
    }
    if expected.is_some_and(|request| {
        request.context() != route.context() || request.height() != route.height()
    }) {
        return Err(Rejected("response_address"));
    }
    journal
...
fn commit(
    ...
    let (outcome, halted) = match committed {
        ...
        Commit::Halted(halt) => (json!({"kind": "halted", "halt": report::halt(halt)}), true),
    };
    Ok((outcome, halted))
}
