...
use super::{
    EMPTY_TERMINAL_TAG, MEMBER_TERMINAL_TAG, NON_MEMBER_TERMINAL_TAG, PATH_STEP_BYTES,
    PROOF_SET_PROOF_MAX_BYTES,
};
use crate::proof_set::{
...
#[derive(Clone, Copy)]
struct TestValue(ProofId);

impl ProofSetValue for TestValue {
...
fn single_bit_id(bit: usize) -> ProofId {
    let mut bytes = [0; 32];
    bytes[bit / 8] = 1 << (7 - bit % 8);
    id(bytes)
}

fn set_for(keys: &[ProofId]) -> AuthenticatedProofSet<TestValue> {
...
fn round_trip(
    ...
    root: ProofSetRoot,
    query: ProofId,
    expected: ProofSetMembership,
) {
    let encoded = proof.to_canonical_bytes();
    let decoded = ProofSetProof::from_canonical_bytes(&encoded).unwrap();
...
fn permute(
    ...
    start: usize,
    query: ProofId,
    expected_root: &mut Option<ProofSetRoot>,
...
fn terminal_and_one_step_encodings_have_stable_goldens() {
    ...
    let empty = AuthenticatedProofSet::<TestValue>::new();
    ...
    assert_eq!(
        ...
        concat!(
            "0100",
            "93e7bd037407e8654873ed319b0130c3117246bd84e184e25dd7d10964a765ed"
        )
    );
    assert_eq!(
...
fn complete_step_prefixes_are_structural_but_partial_steps_are_not() {
    ...
    for length in 0..=member.len() {
        let decoded = ProofSetProof::from_canonical_bytes(&member[..length]);
        if matches!(length, 1 | 34 | 67) {
            let decoded = decoded.unwrap();
            if length == member.len() {
                assert_eq!(decoded.verify(root, zero), Ok(ProofSetMembership::Present));
            } else {
                assert!(matches!(
            ...
            assert_eq!(decoded, Err(ProofSetProofError::UnexpectedEnd));
        }
    }
    ...
    for length in 0..=non_member.len() {
        let decoded = ProofSetProof::from_canonical_bytes(&non_member[..length]);
        if matches!(length, 33 | 66 | 99) {
            let decoded = decoded.unwrap();
            if length == non_member.len() {
                assert_eq!(decoded.verify(root, query), Ok(ProofSetMembership::Absent));
            } else {
                assert!(matches!(
    ...
    let decoded = ProofSetProof::from_canonical_bytes(&extended_member).unwrap();
    assert!(matches!(
    ...
    for suffix_len in 1..PATH_STEP_BYTES {
        ...
        assert_eq!(
            ProofSetProof::from_canonical_bytes(&partial),
            Err(ProofSetProofError::UnexpectedEnd)
        );
    }
}
...
fn decoding_preflights_framing_and_path_shape() {
    assert_eq!(
        ProofSetProof::from_canonical_bytes(&[]),
        Err(ProofSetProofError::UnexpectedEnd)
    );
    for tag in [0x03, 0xff] {
        assert_eq!(
            ProofSetProof::from_canonical_bytes(&[tag]),
            Err(ProofSetProofError::UnknownTerminalTag(tag))
        );
    }
    assert_eq!(
        ProofSetProof::from_canonical_bytes(&[EMPTY_TERMINAL_TAG, 0]),
        Err(ProofSetProofError::TrailingBytes { remaining: 1 })
    );
    for terminal_len in 0..32 {
        let mut bytes = vec![NON_MEMBER_TERMINAL_TAG];
        bytes.resize(1 + terminal_len, 0);
        assert_eq!(
            ProofSetProof::from_canonical_bytes(&bytes),
            Err(ProofSetProofError::UnexpectedEnd)
        );
    }

    let mut oversized = vec![0xff; PROOF_SET_PROOF_MAX_BYTES + 1];
    assert_eq!(
        ProofSetProof::from_canonical_bytes(&oversized),
        Err(ProofSetProofError::InputTooLong {
            actual: PROOF_SET_PROOF_MAX_BYTES + 1,
    ...
    for bits in [[0, 1], [0, 255], [254, 255]] {
        ...
        assert!(ProofSetProof::from_canonical_bytes(&bytes).is_ok());
    }
    ...
    assert_eq!(
        ProofSetProof::from_canonical_bytes(&zero_sibling)
            .unwrap()
            .to_canonical_bytes(),
        zero_sibling
    );
...
fn maximum_member_and_nonmember_proofs_round_trip() {
    let zero = id([0; 32]);
    let mut member_set = AuthenticatedProofSet::new();
    ...
    assert_eq!(member_bytes.len(), PROOF_SET_PROOF_MAX_BYTES);
    round_trip(
        &member,
        member_set.root(),
        zero,
    ...
    let mut non_member_set = AuthenticatedProofSet::new();
    ...
    assert_eq!(non_member_bytes.len(), PROOF_SET_PROOF_MAX_BYTES - 1);
    round_trip(
        &non_member,
        non_member_set.root(),
        query,
    ...
    member_too_long.extend_from_slice(&[0x44; 32]);
    assert!(matches!(
        ProofSetProof::from_canonical_bytes(&member_too_long),
        Err(ProofSetProofError::InputTooLong { .. })
    ));
    ...
    assert!(matches!(
        ProofSetProof::from_canonical_bytes(&non_member_too_long),
        Err(ProofSetProofError::InputTooLong { .. })
    ));
}
...
fn decoded_proofs_still_require_the_original_root_and_query() {
    ...
    let root = set.root();
    let bytes = set.proof(query).to_canonical_bytes();
    let decoded = ProofSetProof::from_canonical_bytes(&bytes).unwrap();

    assert_eq!(decoded.verify(root, query), Ok(ProofSetMembership::Absent));
    assert_eq!(
        decoded.verify(root, zero),
        Err(ProofSetProofError::NonMemberMatchesQuery)
    );

    let mut wrong_root = *root.as_bytes();
    wrong_root[31] ^= 1;
    assert!(matches!(
        decoded.verify(ProofSetRoot::from_bytes(wrong_root), query),
        Err(ProofSetProofError::RootMismatch { .. })
    ));

    let mut changed_terminal = bytes.clone();
    changed_terminal[1] ^= 0x80;
    let changed_terminal = ProofSetProof::from_canonical_bytes(&changed_terminal).unwrap();
    assert_eq!(
        changed_terminal.verify(root, query),
        Err(ProofSetProofError::TerminalPathMismatch { bit: 0 })
    );

    let mut changed_bit = bytes.clone();
    changed_bit[33] = 1;
    let changed_bit = ProofSetProof::from_canonical_bytes(&changed_bit).unwrap();
    assert!(matches!(
    ...
    *changed_sibling_bytes.last_mut().unwrap() ^= 1;
    let changed_sibling = ProofSetProof::from_canonical_bytes(&changed_sibling_bytes).unwrap();
    ...
    let singleton = set_for(&[zero]);
    let singleton_proof =
        ProofSetProof::from_canonical_bytes(&singleton.proof(query).to_canonical_bytes()).unwrap();
