...
pub(super) struct State {
    pub(super) nodes: [Local; 4],
    pub(super) proposals: [u8; 3],
    votes: u64,
}

impl State {
    pub(super) fn vote(self, actor: u8, round: u8, role: u8) -> u8 {
...
pub(super) enum Action {
    Author { actor: u8, proposal: u8 },
    Prevote { actor: u8, proposal: u8 }, // zero explicitly closes Proposal
    Precommit { actor: u8, target: u8 }, // zero explicitly closes Prevote
    Advance { actor: u8, nil_quorum: bool },
    Higher { actor: u8, round: u8, role: u8 },
}

impl Action {
    pub(super) fn actor(self) -> u8 {
        match self {
            Self::Author { actor, .. }
            | Self::Prevote { actor, .. }
...
impl Model {
    pub(super) fn unit(faulty: u8, rules: Rules) -> Self {
        let mut quorums = 0;
    ...
    // The faulty signer can supply a vote of any target; honest presence must
    // come from this execution. Phase progress needs three, catch-up two.
    pub(super) fn present(self, state: State, round: u8, role: u8) -> u32 {
        1 + (0..VALIDATORS)
            .filter(|&actor| actor != self.faulty && state.vote(actor, round, role) != 0)
            .count() as u32
    }
    ...
    pub(super) fn certified_values(self, state: State) -> u8 {
    ...
    fn actions_with_facts(self, state: State, facts: Facts) -> Vec<Action> {
        ...
        for actor in 0..VALIDATORS {
            ...
            if phase == 0 {
                ...
                if self.present(state, round, 0) >= 3 {
                    actions.push(Action::Precommit { actor, target: 0 });
                }
            } else if self.present(state, round, 1) >= 3 {
            ...
            for higher in round + 1..ROUNDS {
                for role in 0..=1 {
                    if self.present(state, higher, role) >= 2 {
    ...
    pub(super) fn apply(self, state: State, action: Action) -> Option<State> {
        ...
        match action {
            ...
            Action::Higher { round, .. } => local.cursor = 3 * round,
        }
        Some(next)
    }
}
...
pub(super) struct Exploration {
    ...
    pub(super) witnesses: BTreeMap<&'static str, Vec<Action>>,
    pub(super) counterexample: Option<Vec<Action>>,
    witness_limit: usize,
}
...
fn observe(
    ...
    match action {
        ...
        Action::Precommit { actor, target } => {
            ...
            if target == 0
                && (0..VALIDATORS).any(|signer| {
                    signer != model.faulty
                        && (1..=2).contains(&before.vote(signer, old.cursor / 3, 0))
                })
            {
                labels.push("mixed_target_prevote_timeout");
            }
        ...
        Action::Higher { role, .. } => {
            labels.push(if role == 0 {
                "higher_prevote"
            } else {
                "higher_precommit"
            });
            // Replay uses exactly one observed honest signer plus the faulty
            // signer; different targets still establish higher-round presence.
            labels.push("higher_two_signers");
        }
        Action::Advance {
            nil_quorum: true, ..
...
pub(super) const WITNESS_LABELS: [&str; 14] = [
    "certificate_a",
    "certificate_b",
    "proof_unlock",
