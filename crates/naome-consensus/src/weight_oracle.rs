...
//! Integer oracles are independent; comparisons exercise shared consensus arithmetic.
...
fn independent_weight_and_schedule_boundaries() {
    ...
    compare_arithmetic(boundary_profile());
    for profile in representatives() {
        assert_eq!(wide_quorums(profile.weights), profile.quorums);
        compare_arithmetic(profile.scaled());
    }
}

// This tests shared arithmetic, not a weighted canonical validator network.
// The live trusted profile continues to require exactly four unit weights.
fn compare_arithmetic(profile: Profile) {
    ...
    let entries: [ActiveAgreementEntry; 4] = std::array::from_fn(|i| {
        ActiveAgreementEntry::new(keys[i], AgreementWeight::new(profile.weights[i]))
    });
    let position = ConsensusPosition::new(ConsensusHeight::new(1), ConsensusRound::new(0));
    let snapshot = ActiveAgreementSnapshot::try_from_preselected(position, &entries).unwrap();
    for mask in 0..16 {
        let signers = keys
            .iter()
            .enumerate()
            ...
            .collect::<Vec<_>>();
        assert_eq!(
            snapshot.has_strict_supermajority(&signers).unwrap(),
            profile.quorums & (1 << mask) != 0,
            "{profile:?} {mask}"
        );
        ...
        assert_eq!(snapshot.signed_weight(&signers).unwrap().units(), signed);
        assert_eq!(
            snapshot.has_strict_one_third(&signers).unwrap(),
