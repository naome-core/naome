...
/// One canonical precommit certificate verified against an exact borrowed
/// active-agreement snapshot.
///
/// Success proves only that every listed active key validly precommitted the
/// same opaque proposal target at the certificate's exact context and snapshot
/// position and that their weight is strictly greater than two thirds of that
/// snapshot's unchanged total. The borrowed snapshot is verification context,
/// not validator-selection or canonical-state authority. This value does not
/// establish proposal validity or availability, finality, selection,
/// persistence, journal mutation, peer trust, signing safety, or economics.
#[derive(Debug, PartialEq, Eq)]
#[must_use]
pub struct VerifiedPrecommitCertificateV0<'snapshot> {
    body: VoteBody,
    entries: Box<[CertificateEntry]>,
    id: PrecommitCertificateId,
...
impl<'snapshot> VerifiedPrecommitCertificateV0<'snapshot> {
    ...
    ///
    /// The certificate must carry one shared precommit body, a nonzero
    ...
    pub fn decode_and_verify(
        ...
        let ConsensusVoteTarget::Proposal(_) = body.target else {
        ...
        hasher.update(bytes);
        let id = PrecommitCertificateId(hasher.finalize().into());
    ...
    pub fn signer_keys(&self) -> impl ExactSizeIterator<Item = ConsensusKey> + '_ {
        self.entries.iter().map(|entry| entry.signer)
    }
