...
use naome_consensus::research::ResearchPhase;
use naome_network::{PeerId, ResearchTicket, StaticArtifactNetwork};
use naome_node::research::{ResearchNode, ResearchNodeError};
use naome_protocol::research_exchange::ResearchRequestBody;
use naome_research::{
...
impl ResearchRuntime {
    pub fn new(
        ...
        let genesis = history
            ...
            .state()
            .genesis();
        let expected = naome_protocol::research_exchange::ResearchContext::new(
            *genesis.id().as_bytes(),
    ...
    pub fn submit_operation(&mut self, operation: SignedOperation) -> Result<OperationId> {
        ...
        let body = naome_research::operations::OperationBody::decode(
            operation.payload(),
            self.state()?.genesis(),
        )?;
        let id = operation.id();
    ...
    fn validate_queue_phase(
        &self,
        body: &naome_research::operations::OperationBody,
        author: naome_research::AccountId,
    ) -> Result<()> {
        use naome_research::{operations::OperationBody, state::Phase};
