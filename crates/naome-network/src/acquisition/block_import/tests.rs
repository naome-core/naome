...
use crate::{Keypair, NetworkEvent, OutboundArtifactFailure};
...
fn reject_block(
    ...
    assert!(!network.has_pending_artifact_for_test());
...
fn reject_exact_payload_without_fallback(
    ...
    assert_snapshot(&directory, &selected, &before);
    assert!(
        (network.pending_count_for_test() == 0),
        "no fallback request may start"
    );
    assert_eq!(network.active_permit_count_for_test(), 0);
...
fn foreign_network_generation_is_rejected_before_block_interpretation() {
    ...
    assert!(
        first
            .pending_block_for_peer_for_test(peer_id)
            .is_some_and(|(id, _)| id == first_request_id)
    );
    assert_eq!(first.active_permit_count_for_test(), 1);
    assert!(first.remove_pending_block_for_test(first_request_id));
...
fn unavailable_block_or_proof_never_selects_partial_state() {
    ...
    assert_snapshot(&directory, &selected, &before);
    assert!((block_network.pending_count_for_test() == 0));
    ...
    assert_snapshot(&directory, &selected, &before);
    assert!(selected.block(block_id).unwrap().is_none());
    assert_eq!(proof_network.active_permit_count_for_test(), 0);
}
