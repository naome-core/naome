...
use crate::{
    Keypair, MAX_PENDING_REQUESTS, MAX_STATIC_PEERS, NetworkEvent, OutboundArtifactFailure,
    PeerSessionEvent, RequestStartError, StaticPeer,
};
...
fn finality_halt_and_reconstruction_limit_precede_payload_request_and_write() {
    ...
    assert_eq!(limit_network.pending_count_for_test(), 0);
...
fn fallback_peer_validation_is_lazy_bounded_and_reports_lowest_raw_identity() {
    ...
    assert!((all_hit_network.pending_count_for_test() == 0));
    assert_eq!(
...
fn fallback_distinguishes_no_requestable_peer_from_last_terminal_exhaustion() {
    ...
    drop(artifact_response_event(&mut network, peers[0], Vec::new()));
    network.mark_connected_for_test(peers[1]);
...
fn fallback_global_request_capacity_is_terminal_without_peer_rotation() {
    ...
    assert_eq!(network.active_permit_count_for_test(), MAX_PENDING_REQUESTS);
    assert!(matches!(
        network.start_artifact_block_candidate_branch_payload_fill_with_peer_fallback(
            &selected,
            &mut candidates,
...
fn fallback_peer_mismatch_unexpected_and_found_failures_do_not_rotate() {
    ...
    assert!((network.pending_count_for_test() == 0));
    assert!((other_network.pending_count_for_test() == 0));
    ...
    assert!((capacity_network.pending_count_for_test() == 0));
