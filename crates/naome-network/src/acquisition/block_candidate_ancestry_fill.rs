...
impl ArtifactBlockCandidateAncestryFillPeers {
    fn validated_fallback(
        ...
        let canonical_peer_ids =
            crate::peer_selection::configured_fallback(peer_ids, MAX_STATIC_PEERS, |peer_id| {
                network.is_configured_peer(peer_id)
            })
            .map_err(|error| match error {
                crate::peer_selection::ConfiguredPeerSetError::Input(
                    crate::peer_selection::PeerSetError::Empty,
                ) => ArtifactBlockCandidateAncestryFillError::EmptyBlockPeerSet,
                crate::peer_selection::ConfiguredPeerSetError::Input(
                    crate::peer_selection::PeerSetError::TooMany { actual, maximum },
                ) => ArtifactBlockCandidateAncestryFillError::TooManyBlockPeers { actual, maximum },
                crate::peer_selection::ConfiguredPeerSetError::Input(
                    crate::peer_selection::PeerSetError::Duplicate(peer_id),
                ) => ArtifactBlockCandidateAncestryFillError::DuplicateBlockPeer { peer_id },
                crate::peer_selection::ConfiguredPeerSetError::Unknown(peer_id) => {
                    ArtifactBlockCandidateAncestryFillError::UnknownBlockPeer { peer_id }
                }
            })?;
        Ok(Self::Fallback(
            ArtifactBlockCandidateAncestryFallbackPeers {
                peer_ids: canonical_peer_ids,
                next_peer_index: 0,
            },
        ))
    }
