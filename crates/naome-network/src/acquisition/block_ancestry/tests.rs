...
use crate::{Keypair, NetworkEvent, RequestStartError};
...
fn next_parent_request_failure_discards_the_unselected_prefix() {
    ...
    assert_eq!(network.active_permit_count_for_test(), 1);
    drop(unrelated);
    let (unrelated_id, _) = pending_block_request(&network, peer_id);
    assert!(network.remove_pending_block_for_test(unrelated_id));
    assert_eq!(network.active_permit_count_for_test(), 0);
    assert_snapshot(&directory, &selected, &before);
}
