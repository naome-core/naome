...
use crate::{InboundArtifactChainHeadRequest, Keypair, PeerSessionEvent, StaticPeer};
...
fn request_id(
    ...
    network
        .pending_head_for_peer_for_test(peer_id)
...
fn response_event(
    ...
    network
        .handle_head_exchange_event_for_test(request_response::Event::Message {
            peer: peer_id,
...
fn failure_event(
    ...
    network
        .handle_head_exchange_event_for_test(request_response::Event::OutboundFailure {
            peer: peer_id,
...
fn shape_failures_start_nothing_and_do_not_advance_request_generation() {
    ...
    assert!((after_failures.pending_count_for_test() == 0));
    assert_eq!(after_failures.active_permit_count_for_test(), 0);
...
fn ordered_peer_preflight_is_all_or_none() {
    ...
    disconnected.mark_disconnected_for_test(second);
    assert!(matches!(
    ...
    assert!((disconnected.pending_count_for_test() == 0));
...
fn cancellation_retains_physical_requests_until_each_terminal_drains() {
    ...
    let ids = peers
        ...
        .collect::<Vec<_>>();
    survey.cancel();
    assert_eq!(network.pending_count_for_test(), MAX_STATIC_PEERS);
    assert_eq!(network.active_permit_count_for_test(), MAX_STATIC_PEERS);
