...
use super::{
    ...
    OutboundArtifactChainHeadFailure, PeerId, RequestStartError, StaticArtifactNetwork,
};
...
impl StaticArtifactNetwork {
    ...
    pub fn start_chain_head_survey(
        ...
        let peers = self
            .start_chain_head_request_batch(peer_ids, request)
            .map_err(|error| match error {
                crate::transport::batch::BatchStartError::RequestStart(source) => {
                    ArtifactChainHeadSurveyStartError::RequestStart(source)
                }
                crate::transport::batch::BatchStartError::InsufficientCapacity { available } => {
                    ArtifactChainHeadSurveyStartError::InsufficientCapacity {
...
fn validate_peer_set(peer_ids: &[PeerId]) -> Result<(), ArtifactChainHeadSurveyStartError> {
    crate::peer_selection::validate_peer_set(peer_ids, MAX_STATIC_PEERS).map_err(
        |error| match error {
            crate::peer_selection::PeerSetError::Empty => {
                ArtifactChainHeadSurveyStartError::EmptyPeerSet
            }
            crate::peer_selection::PeerSetError::TooMany { actual, maximum } => {
                ArtifactChainHeadSurveyStartError::TooManyPeers { actual, maximum }
            }
            crate::peer_selection::PeerSetError::Duplicate(peer_id) => {
                ArtifactChainHeadSurveyStartError::DuplicatePeer(peer_id)
            }
        },
    )
}
