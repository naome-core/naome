...
fn announcement_request_id(
    ...
    let peer_index = network
        ...
        .sessions
        .peer_index(&peer_id)
        .expect("the test peer is configured");
    network
        .pending
        .iter()
        .find_map(|(request_id, pending)| match (request_id, pending) {
            (
                ...
                PendingRequest::Announcement(pending),
            ) if pending.peer_index == peer_index => Some(*request_id),
            _ => None,
        })
...
fn failed_group_start_does_not_advance_the_transport_request_generation() {
    ...
    assert!(after_failure.pending.is_empty());
    assert_eq!(
        after_failure.pending_budget.active.load(Ordering::Relaxed),
        0
    );
...
fn ordered_peer_preflight_rejects_the_group_without_a_queued_prefix() {
    ...
    assert!(disconnected_network.pending.is_empty());
    assert_eq!(
        disconnected_network
            .pending_budget
            .active
    ...
    assert_eq!(occupied_network.pending.len(), 1);
    assert_eq!(
        occupied_network
            .pending_budget
            .active
    ...
    assert!(occupied_network.pending.is_empty());
    assert_eq!(
        occupied_network
            .pending_budget
            .active
...
fn unrelated_cross_network_and_late_generation_events_remain_routable() {
    ...
    assert_eq!(
        first_network.pending_budget.active.load(Ordering::Relaxed),
        0
    );
    assert_eq!(
        second_network.pending_budget.active.load(Ordering::Relaxed),
        0
    );
}
