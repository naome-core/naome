...
use crate::{
    ...
    INBOUND_APPLICATION_REQUEST_BURST, NetworkEvent, OutboundArtifactEvent, PeerId,
};
...
async fn wait_for_closed_channel(
    ...
    timeout(Duration::from_secs(10), async {
        while inbound.is_channel_open_for_test() {
...
async fn indexed_corruption_is_typed_poisoned_and_precedes_closed_channel_and_rate() {
    ...
    .await;
    closed_server.exhaust_application_budget_for_test(Instant::now());
    drop(closed_client);
    ...
    assert_eq!(closed_server.application_tokens_for_test(), 0);
}
...
async fn closed_channel_precedes_the_shared_inbound_token() {
    ...
    server.exhaust_application_budget_for_test(Instant::now());
    drop(client);
...
async fn rate_limit_precedes_payload_read_and_later_admitted_request_detects_corruption() {
    ...
    .await;
    limited_server.exhaust_application_budget_for_test(Instant::now());
    assert!(matches!(
    ...
    .await;
    assert_eq!(
        admitted_server.application_tokens_for_test(),
