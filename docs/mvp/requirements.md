...
- [ ] **AB-10 – Earned validator installation and retirement.** A paid author finalizes an intent, prepares a candidate process, is installed in the oldest stable slot, signs the following record with its selected keys, and receives the corresponding future service share. Verify no-join rotation, an unavailable slot, frozen ongoing ballots, preparation conflicts, old-key rejection, crashes at every custody boundary, catch-up, and four independent replays.
