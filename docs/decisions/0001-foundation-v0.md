...
- assumption-free derivations whose theorem conclusions are explicitly closed;
...
The public `Formula` type is valid by construction. Its internal tree is not directly constructible, so a dangling bound-variable index cannot cross the formula boundary and every schema predicate is already structurally valid.
...
- **Expose a raw formula tree as the primary API:** rejected because every consumer would otherwise need to remember a separate validation step.

## Failure modes addressed

- construction of admitted formulas with dangling bound-variable indices;
...
- the public formula API cannot construct a dangling bound-variable index;
- every fixed ZFC axiom expands to a closed primitive formula;
...
- This change guarantees structural formula construction and checks schema side conditions, not semantic soundness or complete proof derivations.
